...
class Test_Game_Time(ParametrizedTestCaseDevice):
    ...
    def test_01_game_time(self):
        try:
            ...
            self.param.find_element_by_id('com.taptap:id/login_username').set_value(self.TestData['usr1']['username'])
            time.sleep(3)
            self.param.find_element_by_id('com.taptap:id/login_pwd').set_value(self.TestData['usr1']['pwd'])
            time.sleep(3)
            ...
            time.sleep(2)
            self.param.tap([(927, 311)])
            time.sleep(2)
            self.param.tap([(785, 1192)])
            time.sleep(2)
            self.param.back()
            time.sleep(2)
    ...
    def test_02_game_time_partition(self):
        try:
            ...
            while time_control_partition < 30:
                ...
                print "按home键"
                time.sleep(60)
                self.param.start_activity('air.com.bartbonte.yellow', 'air.com.bartbonte.yellow.AppEntry')
                time.sleep(60)
                print "打开游戏"
